    ...
    <div class="panel panel-white">
        <div class="panel-heading">
            <h4 class="panel-title">
                @if(App::isLocale('en'))
                    {{trans('messages.Exit')}} <span class="text-bold">{{trans('messages.Pages')}}</span>
                @else
                    <span class="text-bold">{{trans('messages.Pages')}}</span> {{trans('messages.Exit')}}
                @endif
            </h4>
        </div>
            ...
            <div class="table-responsive">
                <table class="table table-bordered hover" id="pages-table">
                    ...
                    <tr>
                        ...
                        <th style="text-align: center">{{trans('messages.Count')}}</th>
                    </tr>
                    </thead>
                </table>
            </div>
    ...
    <script>
        $(function() {
            $('#pages-table').DataTable({
                ...
                processing: true,
                serverSide: false,
                ajax: '{{ route('exitPages.data') }}',
                columns: [
                    { data: 'url', name: 'url' },
                    { data: 'exitCount', name: 'exitCount' }
                ],
                language: {
