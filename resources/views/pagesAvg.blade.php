...
@section('content')
    <div class="panel panel-white">
        <div class="panel-heading">
            <h4 class="panel-title"><span class="text-bold">{{trans('messages.Pages')}}</span> {{trans('messages.ByAvgLoadingTime')}}</h4>
        </div>
            ...
            <div class="table-responsive">
                <table class="table table-bordered hover" id="pages-table">
                    ...
                    <tr>
                        ...
                        <th style="text-align: center">{{trans('messages.AvgLoadingTime')}}</th>
                    </tr>
                    </thead>
                </table>
            </div>
    ...
    <script>
        $(function() {
            $('#pages-table').DataTable({
                ...
                processing: true,
                serverSide: false,
                ajax: '{{ route('pagesAvg.data') }}',
                columns: [
                    { data: 'url', name: 'url' },
                    { data: 'avgLoadingTime', name: 'avgLoadingTime' }
                ],
                language: {
