do ($ = window.jQuery, window) ->
  ...
  class BootstrapSwitch
    ...
    destroy: ->
      ...
      $form.off("reset.bootstrapSwitch").removeData "bootstrapswitch" if $form.length
      @$container.children().not(@$element).remove()
      @$element.unwrap().unwrap().off(".bootstrapSwitch").removeData "bootstrapswitch"
      @$element
    ...
    _formHandler: ->
      ...
      return if $form.data "bootstrapswitch"
      ...
      .on "reset.bootstrapSwitch", ->
        window.setTimeout ->
          $form
          .find("input")
          .filter( -> $(@).data "bootstrapswitch")
          .each -> $(@).bootstrapSwitch "state", @checked
        , 1
      .data "bootstrapswitch", true
  ...
  $.fn.bootstrapSwitch = (option, args...) ->
    ...
    @each ->
      $this = $ @
      data = $this.data "bootstrapswitch"

      $this.data "bootstrapswitch", data = new BootstrapSwitch @, option unless data
